...
$(document).on('click', '.twitter-header-tab', function (e) {
    ...
    if (PressedTwitterTab !== CurrentTwitterTab) {
        ...
        if (PressedTwitterTab == "twitter-self") {
            $.post('http://qb-phone/GetSelfTweets', JSON.stringify({}), function (selfdata) {
                RL.Phone.Notifications.LoadSelfTweets(selfdata)
            })
        ...
        $.post('http://qb-phone/GetSelfTweets', JSON.stringify({}), function (selfTweets) {
            RL.Phone.Notifications.LoadSelfTweets(selfTweets)
        })
    }
...
RL.Phone.Notifications.LoadSelfTweets = function (Tweets) {
    Tweets = Tweets.reverse();
...
$(document).on('click', '.tweet-delete', function (e) {
    ...
    console.log(id)
    $.post('http://qb-phone/DeleteTweet', JSON.stringify({
        id: id,
    }));

    $.post('http://qb-phone/GetSelfTweets', JSON.stringify({}), function (tweets) {
        RL.Phone.Notifications.LoadSelfTweets(tweets);
    });
    $.post('http://qb-phone/GetTweets', JSON.stringify({}), function (tweets) {
        RL.Phone.Notifications.LoadTweets(tweets);
    });
...
$(document).on('click', '.hashtag-tag-text', function (e) {
    ...
    if (!HashtagOpen) {
        ...
        $.post('http://qb-phone/GetHashtagMessages', JSON.stringify({
            hashtag: Hashtag
...
function CopyMentionTag(elem) {
    ...
    RL.Phone.Notifications.Add("fab fa-twitter", RL.Phone.Functions.Lang("TWITTER_TITLE"), $(elem).data('mentiontag') + " Kopyalandı!", "rgb(27, 149, 224)", 1250);
